...
function listar_datatable()
{
    tabla = $('#tbllistado').dataTable({
        ...
        "ajax":
        {
            url: urlweb + '?c=Artistas&a=listar', 
            dataType : "json",
            error: function(e)
            {
...
function desactivar(art_estado, art_id)
{
    mensaje_confirmacion.fire(
        ...
        }).then((result) =>
        {
            ...
            {
                $.ajax({
                    url : urlweb + '?c=Artistas&a=activar_desactivar',
                    type : 'POST',
                    data : {art_id : art_id, art_estado : art_estado},
                    dataType: 'json',
                    beforeSend: function()
                    {
...
function activar(art_estado, art_id)
{
    mensaje_confirmacion.fire(
        { 
            icon : 'info',
...
{
    $.ajax({
        url: urlweb + '?c=Artistas&a=obtener',
        type: 'POST',
        dataType: 'json',
        data:{art_id: art_id},
        beforeSend: function()
    ...
    }).done(function(data, textStatus, jqXHR)
    {
        ...
        $('#art_genero').val(data.art_genero);
        $('#art_descripcion').val(data.art_descripcion);
        $('#myModal').modal('show');
        $('#temp_img1').val(data.art_imagen_logo);
        if(data.art_imagen_logo != "")
        {
            file_1.addFile(urlweb + "media/artistas_logo/"+data.art_imagen_logo+"?file"+new Date().getTime());
        }

        $('#temp_img2').val(data.art_imagen_portada);
        if(data.art_imagen_portada != "")
        {
            file_2.addFile(urlweb + "media/artistas_portada/"+data.art_imagen_portada+"?file2"+new Date().getTime());
        }
...
function guardar_editar(e) 
{
    if(validar_formulario().form())
    {
        ...
        if($('#art_id').val() != "") { isNuevo = "Se procedera a editar la categoria"; mensaje_sec = "Editando..."  }
        mensaje_confirmacion.fire(
            { 
                icon : 'info',
                title: 'Necesitamos de tu Confirmación',
            ...
            }).then((result) =>
            {
                if (result.value == true)
                {
                    ...
                    $.ajax({
                        url : urlweb + "?c=Artistas&a=guardar_editar",
                        type : "POST",
                        dataType: 'json',
                        data : formdata,
                        contentType : false,
...
function eliminar(art_id)
{
    mensaje_confirmacion.fire(
        ...
        }).then((result) =>
        {
            ...
            {
                $.ajax({
                    url : urlweb + '?c=Artistas&a=eliminar',
                    type : 'POST',
                    data : {art_id : art_id},
                    dataType: 'json',
                    beforeSend: function()
